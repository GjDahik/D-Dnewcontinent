...
function loadLoginPlayers() {
    ...
    // Primero autenticar, después leer Firestore (esperar auth listo antes de la query).
    var p = (typeof window.__authReadyPromise !== 'undefined') ? window.__authReadyPromise : Promise.resolve();
    p.then(doQuery).catch(function () { doQuery(); });
}
